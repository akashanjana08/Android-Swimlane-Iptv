...
final public class MyGridLayoutManager extends RecyclerView.LayoutManager {
    ...
    final static class LayoutParams extends RecyclerView.LayoutParams {
        ...
        private MyItemAlignmentFacet mAlignmentFacet;
        ...
        void setItemAlignmentFacet(MyItemAlignmentFacet facet) {
            mAlignmentFacet = facet;
        }

        MyItemAlignmentFacet getItemAlignmentFacet() {
            return mAlignmentFacet;
        }

        void calculateItemAlignments(int orientation, View view) {
            MyItemAlignmentFacet.ItemAlignmentDef[] defs = mAlignmentFacet.getAlignmentDefs();
            if (mAlignMultiple == null || mAlignMultiple.length != defs.length) {
                mAlignMultiple = new int[defs.length];
            }
            for (int i = 0; i < defs.length; i++) {
                mAlignMultiple[i] = MyItemAlignmentFacetHelper
        ...
        protected void onTargetFound(View targetView,
            ...
            if (getScrollPosition(targetView, null, sTwoInts)) {
                ...
                if (mOrientation == HORIZONTAL) {
                    dx = sTwoInts[0];
                    dy = sTwoInts[1];
                    System.out.println("onTargetFound_scrollX :"+ dx);
                    System.out.println("onTargetFound_scrollY :"+ dy);
    ...
    final MyBaseGridView mBaseGridView;
     ...
     */
    private final MyItemAlignment mItemAlignment = new MyItemAlignment();
    ...
    public MyGridLayoutManager(MyBaseGridView baseGridView) {
    ...
    int getSubPositionByView(View view, View childView) {
        ...
        final MyItemAlignmentFacet facet = lp.getItemAlignmentFacet();
        if (facet != null) {
            final MyItemAlignmentFacet.ItemAlignmentDef[] defs = facet.getAlignmentDefs();
            if (defs.length > 1) {
    ...
    private Grid.Provider mGridProvider = new Grid.Provider() {
        ...
        public int createItem(int index, boolean append, Object[] item) {
            ...
            lp.setItemAlignmentFacet((MyItemAlignmentFacet)getFacet(vh, MyItemAlignmentFacet.class));
    ...
    private void updateChildAlignments(View v) {
        ...
        if (p.getItemAlignmentFacet() == null) {
            ...
            // Use MyItemAlignmentFacet defined on specific ViewHolder
    ...
    void updateScrollMax() {
        ...
        ((Activity)mBaseGridView.getContext()).getWindowManager().getDefaultDisplay().getMetrics(displayMetrics);
        int width = displayMetrics.widthPixels;

        //int maxScroll =  getPrimarySystemScrollPositionOfChildMax(findViewByPosition(pos)) + 1600;

        int child = mBaseGridView.getChildCount();
        int maxScroll =  width + ((findViewByPosition(pos).getWidth()*(mBaseGridView.getLayoutManager().getItemCount()-7)) -40) ;
    ...
    int startPositionSmoothScroller(int position) {
        LinearSmoothScroller linearSmoothScroller = new LinearSmoothScroller(mBaseGridView.getContext()) {
            @Override
            public PointF computeScrollVectorForPosition(int targetPosition) {
                if (getChildCount() == 0) {
                    return null;
        ...
        getPrimarySystemScrollPosition(findViewByPosition(position));
        startSmoothScroll(linearSmoothScroller);
        int targetPos = linearSmoothScroller.getTargetPosition();
        return targetPos;
    }
    ...
    /**
     * Get adjusted primary position for a given childView (if there is multiple MyItemAlignment defined
     * on the view).
     */
    ...
    private boolean getNoneAlignedPosition(View view, int[] deltas) {
        System.out.println("getAlignedPosition===================");
        int pos = getPositionByView(view);
    ...
    static public int DPAD_DIRECTIONAL=0;
    private void scrollGrid(int scrollPrimary, int scrollSecondary, boolean smooth) {
        if (mInLayout) {
            ...
            if (mOrientation == HORIZONTAL) {
                ...
                if(DPAD_DIRECTIONAL==1)
                {
                    scrollXPosition = findViewByPosition(mFocusPosition).getWidth();
                }
                else {
                    scrollXPosition = - (findViewByPosition(mFocusPosition).getWidth());
                }
            ...
            if (smooth) {
                ...
                System.out.println("smooth: TRUE");
            } else {
                mBaseGridView.scrollBy(scrollX, scrollY);
                //dispatchChildSelectedAndPositioned();
                System.out.println("smooth: FALSE");
            }
        }
    }
    ...
    final static class SavedState implements Parcelable {
        ...
        SavedState(Parcel in) {
            index = in.readInt();
            childStates = in.readBundle(MyGridLayoutManager.class.getClassLoader());
        }
